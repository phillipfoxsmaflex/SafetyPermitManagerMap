...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/settings", requireAuth, async (req, res) => {
    try {
      const user = req.user;
      if (user.role !== 'admin') {
        return res.status(403).json({ message: "Nur Administratoren können App-Einstellungen ändern" });
      }
  ...
  app.post("/api/settings/logo", requireAuth, upload.single('logo'), async (req, res) => {
    try {
      const user = req.user;
      if (user.role !== 'admin') {
        return res.status(403).json({ message: "Nur Administratoren können das Logo ändern" });
      }
      ...
      if (!allowedMimes.includes(req.file.mimetype)) {
        return res.status(400).json({ message: "Nur Bilddateien sind erlaubt (JPEG, PNG, GIF, WebP)" });
      }

      // Save logo path to settings
      const logoPath = `/uploads/${req.file.filename}`;
      const settings = await storage.updateAppSettings({ logoPath, updatedBy: user.id });
